...
Each tab's arrangement is saved per browser and restored on the next visit.
...
1. On opening a tab, the saved layout (if any) is **reconciled** against the widgets currently available: known widgets
   ...
   to fit), newly-available widgets are placed, and rects for widgets that disappeared are removed.
...
3. Edits (move, resize, reset) update the stored layout, which **persists locally** and syncs across windows.

## Related

- **Widget injection** — each widget is injected into a tab's slot and declares its `defaultSize` / `minSize` /
  `maxSize` there; the grid reads these to size and place it.
