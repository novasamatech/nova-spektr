...
import { entries, nonNullable, nullable } from '@/shared/lib/utils';
import { type AccountId } from '@/shared/polkadotjs-schemas';
import { type AccountIdentity, identity } from '@/domains/network';
import {
  type EraValidator,
...
const $identityParents = combine($chainId, identity.$list, (chainId, identities) => {
  ...
  return nullable(chainIdentities) ? EMPTY_PARENTS : buildIdentityParents(chainIdentities);
});
...
/**
 * Groups validators into operator clusters keyed by their on-chain identity
 * display name - every sub-identity of one operator carries the parent's
 * display name, which is what "3rd validator of the same operator" means to a
 * user reading the list.
 *
 * `IdentityParentMap` values are account ids, so each group elects a stable
 * representative (the lowest account id of the group) as its key. Validators
 * with no identity get no entry and are therefore never clustered.
 */
function buildIdentityParents(identities: Record<AccountId, AccountIdentity>): IdentityParentMap {
  const groups = new Map<string, AccountId[]>();

  for (const [accountId, accountIdentity] of entries(identities)) {
    const name = accountIdentity.name.trim().toLowerCase();
