...
import type { AccountId, DraftAccount, ShardAccount, ChainAccount, Account_NEW } from '@shared/core';
...
export const DerivationsAddressModal = ({ isOpen, rootAccountId, keys, onClose, onComplete }: Props) => {
  ...
  const handleScanResult = (result: DdAddressInfoDecoded[]) => {
    ...
    const newAccounts = accounts.filter((account) => !(account as Account_NEW).id);
