...
/**
 * One table state for the Locks card and its full-screen modal: both read the
 * same rows and dispatch the same click, and a filter set in the modal is still
 * in effect the next time it opens. Call it once, in the card, and pass the
 * state down — a second call site would get its own filters and its own data
 * snapshot.
 */
