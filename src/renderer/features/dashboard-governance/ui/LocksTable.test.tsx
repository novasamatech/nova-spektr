...
import { makeLockRow as row, makeLocksTableState as state } from '../lib/__tests__/fixtures';
...
describe('features/dashboard-governance/ui/LocksTable', () => {
  it('shows only Account, Locked and Action in compact mode', () => {
    renderTable('compact', state({ rows: [row()] }));
  ...
  it('shows the full column set in full mode, without empty Pending and Delegated', () => {
    renderTable('full', state({ rows: [row()] }));
  ...
  it('brings Pending and Delegated back in full mode when a row has them', () => {
    renderTable('full', state({ rows: [row({ pending: new BN(5), delegated: new BN(5) })] }));
  ...
  it('captions the compact Locked cell with the release line when nothing is claimable', () => {
    renderTable(
      'compact',
      state({ rows: [row({ claimable: BN_ZERO, claimableActions: [], pending: new BN(5), nextUnlockAtMs: null })] }),
    );
  ...
  it('captions the compact Locked cell with the delegated amount when that is all there is', () => {
    renderTable(
      'compact',
      state({ rows: [row({ claimable: BN_ZERO, claimableActions: [], delegated: new BN('1000000000000') })] }),
    );
