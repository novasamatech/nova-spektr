...
const FiltersForm = ({ onDone }: FormProps) => {
  ...
  const { filters, asset, filtersDiffer } = useUnit({
    filters: validatorSelectionModel.$filters,
    asset: validatorSelectionModel.$asset,
    filtersDiffer: validatorSelectionModel.$filtersDiffer,
  });
  ...
  // The text fields hold their own draft state, so resetting the store alone
  // would leave the typed bounds on screen while the list stopped honouring
  // them.
  const handleReset = () => {
    setMinApy('');
    setMaxCommission('');
  ...
  return (
    <div className="flex w-72 flex-col gap-y-4 p-4">
      ...
      <div className="flex items-center gap-x-2">
        {/*
          Disabled rather than hidden while the filters are already at their
          defaults: a button that appears and disappears under the pointer is
          harder to aim at than one that is simply inert.
        */}
        <Button variant="text" size="sm" disabled={!filtersDiffer} onClick={handleReset}>
          {t('staking.validatorSelection.filters.reset')}
        </Button>
        <Button className="flex-1" size="sm" onClick={onDone}>
