...
import { type DisplayedStrings, type ScoreMap } from '../types';
...
describe('sorting by score', () => {
  const score = (overall: number) => ({ apy: 0, commission: 0, selfStake: 0, eraPoints: 0, overall });

  const scores: ScoreMap = {
    [accountId(1)]: score(0.2),
    [accountId(2)]: score(0.9),
    [accountId(3)]: score(0.5),
  };

  it('orders by the composite score in both directions', () => {
    const validators = [makeValidator(1), makeValidator(2), makeValidator(3)];

    expect(ids(sortValidators(validators, { column: 'score', direction: 'desc' }, undefined, scores))).toEqual([
      accountId(2),
      accountId(3),
      accountId(1),
    ]);
    expect(ids(sortValidators(validators, { column: 'score', direction: 'asc' }, undefined, scores))).toEqual([
      accountId(1),
      accountId(3),
      accountId(2),
    ]);
  ...
  it('treats a validator outside the scored set as a zero, not as unknown', () => {
    // The scores are built from the same elected set the table renders, so a
    // missing entry means "not elected", not "not measured".
    const validators = [makeValidator(1), makeValidator(9)];
  ...
  it('keeps the incoming order when no scores are supplied at all', () => {
    const validators = [makeValidator(3), makeValidator(1), makeValidator(2)];

    expect(ids(sortValidators(validators, { column: 'score', direction: 'desc' }))).toEqual([
      accountId(3),
      accountId(1),
      accountId(2),
    ]);
