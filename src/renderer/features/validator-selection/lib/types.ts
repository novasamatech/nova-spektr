...
export type SortColumn = 'validator' | 'score' | 'eraPoints' | 'nominators' | 'ownStake' | 'commission' | 'apy';
...
/**
 * Recommendation scores of the elected set, keyed by account id. Built once
 * against the whole set, so a row's score does not move when the user filters
 * or searches.
 */
export type ScoreMap = Record<AccountId, ScoreBreakdown>;
