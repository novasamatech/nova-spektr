...
/**
 * The same amount without the thousands shorthand — for the drill-downs, which
 * are the "one click away" the card's abbreviation promises.
 *
 * `0.1K DOT` under a card is a reasonable compression of ninety-four tokens; in
 * a table row it reads as a rounding error, and a user comparing ten rows of
 * `0.1K` cannot tell which is the biggest. Millions keep their `M`: nobody
 * needs eight digits to compare positions.
 */
export function formatAssetAmountExact({ amount, precision, symbol }: AssetAmount): string {
