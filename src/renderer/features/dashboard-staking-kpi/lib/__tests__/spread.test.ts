...
import { buildDistribution, buildSpreadRows, countSpread, toAllocationRows } from '../spread';
...
describe('countSpread', () => {
  it('counts statuses and the accounts behind them', () => {
    const rows = buildSpreadRows({
      positions: [
        position({
          accountId: account(1),
          nominations: [validator(1), validator(2)],
          activeValidators: [validator(1)],
        }),
        position({ accountId: account(2), nominations: [validator(3)] }),
      ],
      exposuresByChain: exposures({ [validator(1)]: [{ who: account(1), value: '600' }] }),
      eraValidatorsByChain: elected([validator(1), validator(2)]),
      metaByChain: META,
    ...
    expect(countSpread(rows)).toEqual({ total: 3, active: 1, droppedOut: 1, waiting: 1, accounts: 2 });
  });
});
...
describe('buildDistribution', () => {
  ...
  it('sums a validator across the accounts backing it', () => {
    const positions = [
      position({ accountId: account(1), nominations: [validator(1)], activeValidators: [validator(1)], total: '600' }),
      position({ accountId: account(2), nominations: [validator(1)], activeValidators: [validator(1)], total: '400' }),
    ];
    ...
    const rows = buildSpreadRows({
      ...
      exposuresByChain: exposures({
        [validator(1)]: [
          { who: account(1), value: '600' },
          { who: account(2), value: '400' },
        ],
      }),
      eraValidatorsByChain: elected([validator(1)]),
      metaByChain: META,
    ...
    const { slices, idleFiat } = buildDistribution({ rows, positions, toFiat });

    expect(slices).toHaveLength(1);
    expect(slices[0]).toMatchObject({ allocated: '1000', fiat: '1000', accountCount: 2 });
    expect(idleFiat).toBe('0');
  });

  it('counts the bond of a position the election passed over as idle', () => {
    const positions = [
      position({ accountId: account(1), nominations: [validator(1)], activeValidators: [validator(1)], total: '600' }),
      position({ accountId: account(2), nominations: [validator(2)], total: '400' }),
    ];
    ...
    const { idleFiat, totalFiat } = buildDistribution({ rows, positions, toFiat });

    expect(idleFiat).toBe('400');
    expect(totalFiat).toBe('1000');
  });

  it('counts the part of a bond the election left over', () => {
    // Bonded 1000, but only 600 of it ended up behind a validator.
    const positions = [
      position({ accountId: account(1), nominations: [validator(1)], activeValidators: [validator(1)], total: '1000' }),
    ];
    ...
    expect(buildDistribution({ rows, positions, toFiat }).idleFiat).toBe('400');
  });

  it('says nothing about a position whose allocation is unknown', () => {
    const positions = [
      position({ accountId: account(1), nominations: [validator(1)], activeValidators: [validator(1)], total: '1000' }),
    ];
    ...
    const rows = buildSpreadRows({
      positions,
      exposuresByChain: exposures({ [validator(2)]: [{ who: account(2), value: '1' }] }),
      eraValidatorsByChain: elected([validator(1)]),
      metaByChain: META,
    });

    expect(buildDistribution({ rows, positions, toFiat }).idleFiat).toBe('0');
  });

  it('ranks slices by value, not by raw planck', () => {
    // KSM has 12 decimals to DOT's 10: ranked by planck the smaller position
    // would come first, which is exactly what fiat is here to prevent.
    const positions = [
      position({ accountId: account(1), nominations: [validator(1)], activeValidators: [validator(1)], total: '100' }),
      position({
        accountId: account(2),
        chainId: KUSAMA,
        nominations: [validator(2)],
        activeValidators: [validator(2)],
        total: '10000',
      }),
    ];
    ...
    const rows = buildSpreadRows({
      positions,
      exposuresByChain: {
        ...exposures({ [validator(1)]: [{ who: account(1), value: '100' }] }),
        ...exposures({ [validator(2)]: [{ who: account(2), value: '10000' }] }, KUSAMA),
      },
      eraValidatorsByChain: { ...elected([validator(1)]), ...elected([validator(2)], KUSAMA) },
      metaByChain: META,
    });

    // A price feed that values the DOT position above the larger KSM planck.
    const priced = (chainId: ChainId, planck: string) => (chainId === POLKADOT ? `${Number(planck) * 1000}` : planck);

    expect(buildDistribution({ rows, positions, toFiat: priced }).slices.map((s) => s.validatorId)).toEqual([
      validator(1),
      validator(2),
    ]);
  });
