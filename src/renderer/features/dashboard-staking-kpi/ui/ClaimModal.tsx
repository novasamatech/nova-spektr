...
import { formatAssetAmount, formatAssetAmounts } from '../lib/amounts';
import { type CsvClaimRow, claimCsvColumns, csvFileName } from '../lib/csv';
import { CLAIM_WINDOW_ERAS, daysUntilExpiry, erasUntilExpiry, oldestPayoutEra } from '../lib/expiry';
...
export const ClaimModal = memo(({ rows, currency, eras, eraDurations, walletByAccount, onClose }: Props) => {
    ...
    return rows.map((row) => {
      ...
      const expiryDays =
        oldest === null || activeEra === undefined
          ? null
          : daysUntilExpiry(erasUntilExpiry(oldest, activeEra), eraDurations[row.chainId] ?? null);

      return { ...row, selected: selected.has(row.key), expiryDays };
    });
  }, [rows, eras, eraDurations, selected]);
  ...
  const unclaimedTotalFiat = useMemo(
    () => rows.reduce((sum, row) => sum + Number(row.unclaimedFiat), 0).toString(),
    [rows],
  );
  ...
  const columns = useMemo<Column<TableRow>[]>(
    ...
    [t, chains, walletByAccount, currency, selectAllState, rows, handleToggleAll, handleToggleRow],
  );
