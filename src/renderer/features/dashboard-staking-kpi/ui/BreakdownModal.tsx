...
export type BreakdownMode = 'apy' | 'nominations';
...
/**
 * The APY and Active-nominations cards open the same breakdown: one donut over
 * the positions plus a row per position, hover-linked in both directions.
 */
export const BreakdownModal = memo(
  ({ mode, rows, currency, totalFiat, headline, headlineClass, walletByAccount, onClose }: Props) => {
    const { t } = useI18n();
    ...
    const title =
      mode === 'apy' ? t('dashboard.staking.kpi.apy.detailTitle') : t('dashboard.staking.kpi.nominations.detailTitle');
                        ...
                        <div className="shrink-0 text-right">
                          {mode === 'apy' ? (
                            ...
                            <FootnoteText className="tabular-nums">
                              {t('dashboard.staking.kpi.nominations.count', { count: row.validatorCount })}
                            </FootnoteText>
                          )}
