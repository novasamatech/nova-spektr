...
import { $primaryIpfsGateway, evidenceService, memberService, salaryService } from '@/domains/collectives';
...
export const PromotionInfo = memo(() => {
  ...
  const openEvidence = () => {
    if (evidence) {
      window.open(evidenceService.getEvidenceFetchIpfsUrl(evidence.hash, primaryGateway), '_blank');
    }
  };
