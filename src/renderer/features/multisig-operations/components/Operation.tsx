...
const compareTxWithoutDeposit = (oldTx: MultisigOperation, newTx: MultisigOperation) => {
  const { deposit: _oldDeposit, ...oldWithoutDeposit } = oldTx;
  const { deposit: _newDeposit, ...newWithoutDeposit } = newTx;

  return isEqual(oldWithoutDeposit, newWithoutDeposit);
};
...
const Operation = memo(
  ...
  (previousePros, props) => {
    const equalTx = compareTxWithoutDeposit(previousePros.tx, props.tx);
    const equalAccount = isEqual(previousePros.account, props.account);

    return equalTx && equalAccount;
  },
);
