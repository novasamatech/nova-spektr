...
export const RestakeFormShards = ({ onGoBack }: Props) => {
  const { submit } = useForm(formModelShards.$restakeForm);
...
const ProxyFeeAlert = () => {
  const {
    fields: { shards },
  } = useForm(formModelShards.$restakeForm);
...
const Amount = () => {
  ...
  const restakeBalanceRange = useUnit(formModelShards.$restakeBalanceRange);
