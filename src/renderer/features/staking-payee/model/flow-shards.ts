...
import { type FeeData, type PayeeDataShards, type WalletDataShards, Step } from '../lib/types';
...
const $walletData = restore<WalletDataShards | null>(flowStarted, null).reset(flowFinished);
const $payeeData = createStore<PayeeDataShards | null>(null).reset(flowFinished);
...
sample({
  clock: [flowStarted, formModelShards.output.formChanged],
  source: {
    walletData: $walletData,
...
sample({
  ...
  target: formModelShards.events.txWrapperChanged,
});

sample({
  clock: formModelShards.output.formChanged,
  target: $payeeData,
});
...
sample({
  ...
  target: formModelShards.events.isFeeLoadingChanged,
});
...
sample({
  ...
  target: formModelShards.events.feeDataChanged,
});
...
sample({
  clock: flowStarted,
  target: formModelShards.events.formInitiated,
});
...
sample({
  clock: flowFinished,
  fn: () => Step.NONE,
  target: [stepChanged, formModelShards.events.formCleared],
});
