...
import { type Chain, type ChainId, type EraIndex, ExternalType } from '@/shared/core';
import { stakingPallet } from '@/shared/pallet/staking';
import { type PayoutsResourceParams, type RewardSource, rewardsService } from '@/domains/staking';
import { type ClaimedRewards } from '../types';

function collectRewardSources(chain: Chain, chainsMap: Record<ChainId, Chain>): RewardSource[] {
...
export function buildPayoutRefetchParams({ claimed, apis, chains, eras }: Params): PayoutsResourceParams[] {
  ...
  const rewardSources = collectRewardSources(claimed.chain, chains);
