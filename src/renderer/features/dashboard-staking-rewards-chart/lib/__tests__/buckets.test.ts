...
describe('value label visibility', () => {
  test.each([
    ...
    ['90d', true],
    ['1y', true],
  ] as const)('%s shows labels: %s', (range, expected) => {
    expect(shouldShowValueLabels(RANGE_CONFIG[range].count)).toEqual(expected);
  });

  test('the cut-off is thirteen bars inclusive', () => {
    expect(shouldShowValueLabels(13)).toEqual(true);
    expect(shouldShowValueLabels(14)).toEqual(false);
  });

  test('no bars means nothing to label', () => {
    expect(shouldShowValueLabels(0)).toEqual(false);
  });
});
