...
type Props = {
  ...
  className?: string;
  onStart?: () => void;
  onResult: (scanResult: Uint8Array[]) => void;
  onError?: (error: ErrorObject) => void;
  onProgress?: (progress: Progress) => void;
  onCameraList?: (cameras: VideoInput[]) => void;
};

const MultiframeSignatureQrReader = ({
  size = 300,
  cameraId,
  className,
  onCameraList,
  ...
  const makeResultPayload = <T extends Uint8Array[]>(data: T): Uint8Array[] => {
    return data;
  };
  ...
  const handleFirstFrame = (
    ...
    if (fountainResult) {
      ...
      onResult?.(makeResultPayload<Uint8Array[]>(result.payload));
    } else {
  ...
  const handleNextFrames = (raptorDecoder: Decoder, blockNumber: number, newSize: number) => {
    ...
    for (const [_, packet] of packets.current) {
      ...
      console.log(`fountain result ${fountainResult}`);
      let result = {};
      try {
        result = SIGNED_TRANSACTION_BULK.decode(fountainResult);
      } catch (e) {
...
export default MultiframeSignatureQrReader;
