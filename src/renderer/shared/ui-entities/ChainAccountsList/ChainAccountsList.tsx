...
export const ChainAccountsList = memo(({ accounts }: Props) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <ScrollArea>
        <ul className="divide-divider flex flex-col divide-y px-5 pb-3">
...
const ListItem = memo(({ chain, accountId }: { chain: Chain; accountId: AccountId }) => {
  return (
    <li className="text-footnote flex min-w-0 items-center gap-1 py-2">
      <div className="flex-1 basis-1/3">
        <ChainTitle fontClass="text-text-primary" chain={chain} />
      </div>
      <div className="text-text-secondary flex min-w-0 flex-1 basis-2/3">
