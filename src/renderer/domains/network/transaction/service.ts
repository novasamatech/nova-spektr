...
function getInnerCallsFromCall(call: Call, proxyTarget?: AccountId, depth = 0): Call[] {
  ...
  if (call.section === 'utility') {
    ...
    if (call.method === 'withWeight') {
      return getInnerCallsFromCall(call.args[0] as unknown as Call, proxyTarget, next);
    }
    if (call.method === 'dispatchAs' || call.method === 'asDerivative') {
      return getInnerCallsFromCall(call.args[1] as unknown as Call, proxyTarget, next);
    }
  }

  if (proxyTarget && call.section === 'proxy' && call.method === 'proxy') {
    const realArg = call.args[0] as unknown as { isId?: boolean; asId?: { toHex(): string }; toHex?: () => string };
    const real = realArg?.isId ? realArg.asId?.toHex() : realArg?.toHex?.();
    if (real === proxyTarget) {
      return getInnerCallsFromCall(call.args[2] as unknown as Call, proxyTarget, next);
    }
  }
