 ...
 */
function getGeneratedAccountNames(accountId: AccountId, addressPrefix: number | undefined): string[] {
 ...
 * `Main...Vault`.
 */
function isGeneratedAccountName(name: string, accountId: AccountId, addressPrefix: number | undefined): boolean {
...
function isWalletNameAutoGenerated({
  ...
  if (walletAccount.nameType === AccountNameType.CUSTOM) {
    const prefix = getAccountAddressPrefix(undefined, walletAccount, chains);
    if (!isGeneratedAccountName(walletAccount.name, accountId, prefix)) {
      return false;
    }
  }
