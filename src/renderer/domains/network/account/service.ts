...
function resolveWalletName({ wallet, accounts, contacts, identities, chains }: ResolveWalletNameParams): string {
  ...
  const prefix = getAccountAddressPrefix(undefined, walletAccount, chains);

  if (isUserChosenAccountName(walletAccount, prefix)) {
    return walletAccount.name;
  }

  const localContact = findNamedLocalContact(contacts, accountId, prefix);
  if (localContact) {
...
/**
 * The local contact that names this account. A local contact whose name is just
 * the account's own short address names nothing: older multisig-create flows
 * saved a signatory's auto-filled stored name as a contact, which for a synced
 * account is that short address, and it would otherwise hide the address book
 * entry for the same account forever.
 */
function findNamedLocalContact(
  contacts: Contact[],
  accountId: AccountId,
  addressPrefix: number | undefined,
): Contact | undefined {
  return contacts.find(
    c => c.accountId === accountId && isLocalContact(c) && !isGeneratedAccountName(c.name, accountId, addressPrefix),
  );
}

/**
 * Whether the stored name is one the user actually chose. A CUSTOM stamp alone
 * cannot answer that on older profiles (see `isGeneratedAccountName`): a CUSTOM
 * name that regenerates from the account's own address is not user-chosen and
 * must not shadow the address book.
 */
function isUserChosenAccountName(account: AnyAccount, addressPrefix: number | undefined): boolean {
  return isCustomAccountName(account) && !isGeneratedAccountName(account.name, account.accountId, addressPrefix);
}
...
function isWalletNameAutoGenerated({
  ...
  identities,
  chains = {},
}: Omit<ResolveWalletNameParams, 'chains'> & Partial<Pick<ResolveWalletNameParams, 'chains'>>): boolean {
  const walletAccounts = filterAccountsByWallet(accounts, wallet.id);
  ...
  const hasBackendContact = contacts.some(c => c.accountId === accountId && isBackendContact(c));
  if (hasBackendContact || findNamedLocalContact(contacts, accountId, prefix)) {
    return false;
  }
...
function resolveAccountName({
  ...
  // and a fallback that is itself this account's generated name is no better
  // than what follows, so skip it.
  if (fallbackName && !isGeneratedAccountName(fallbackName, accountId, prefix)) {
    return fallbackName;
  }
