...
// The era length itself now has a single implementation shared with the era
// anchor — see `era/__tests__/eraDuration.test.ts`.
describe('era duration used for annualisation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    consts.sessionsPerEra.mockReturnValue(6);
    storage.erasTotalStake.mockResolvedValue(new BN(TOTAL_STAKED));
  ...
  test('halves the APY when the era is twice as long', async () => {
    const apy = await apyService.getNetworkApy({
      api: mockApi(),
      // 48h era: twice the 24h baseline → half the eras per year.
      timelineApi: relayApi(4800),
      chain: polkadotAh,
      era: 100,
      validators: validators(0),
    });

    expect(apy).toEqual('2.72');
  });

  test('uses the per-chain fallback era length when no relay api is connected', async () => {
    const apy = await apyService.getNetworkApy({
      api: mockApi(),
      timelineApi: null,
      // Kusama Asset Hub falls back to a 6h era → 4× the eras per year.
      chain: { chainId: AssetHubChains.KUSAMA_AH } as Chain,
      era: 100,
      validators: validators(0),
    });

    expect(apy).toEqual('21.76');
  });
});
