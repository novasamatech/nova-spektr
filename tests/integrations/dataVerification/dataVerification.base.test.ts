...
describe('Verification function can verify parachains', () => {
  ...
  test.skip.each(polkadotParachains)('Can verify data for polkadot parachain: $name', async (parachain) => {
  ...
  test.skip.each(kusamaParachains)('Can verify data for kusama parachain: $name', async (parachain) => {
    const parachainApi = await createWsConnection(parachain.nodes[0].url);
    const parachainAccount = testAccounts.find((data) => data.chainId == parachain.chainId.slice(2));
    const storageKey = polkadotApi.query.system.account.key(parachainAccount?.account);
  ...
  test.skip.each(kusamaParachains)('Verification return false if nonce was changed for $name', async (parachain) => {
    const parachainApi = await createWsConnection(parachain.nodes[0].url);
    const parachainAccount = testAccounts.find((data) => data.chainId == parachain.chainId.slice(2));
    const storageKey = polkadotApi.query.system.account.key(parachainAccount?.account);
  ...
  test.skip.each(polkadotParachains)(
    'Verification return false if balance was changed for $name',
    async (parachain) => {
      const parachainApi = await createWsConnection(parachain.nodes[0].url);
      const parachainAccount = testAccounts.find((data) => data.chainId == parachain.chainId.slice(2));
      const storageKey = polkadotApi.query.system.account.key(parachainAccount?.account);
