...
function createTarget(account: AnyAccount | null, accountId: AccountId): AmountFlowTarget {
  return {
...
describe('Staking Amount Flow - Route Signer Guard', () => {
  ...
  it('should block the flow for a watch-only account', async () => {
    ...
    await env.executeEvent(amountFlowModel.unbondRequested, createTarget(watchOnlyAccount, watchOnlyAccount.accountId));
  ...
  it('should block the flow for a contact position (no local account)', async () => {
    await buildEnv([stakingAccountA]);

    await env.executeEvent(amountFlowModel.unbondRequested, createTarget(null, contactAccountId));
