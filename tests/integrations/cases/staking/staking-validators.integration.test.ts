...
describe('Staking Validators - Integration', () => {
  ...
  describe('recommendation', () => {
    ...
    it('should never fill more nomination slots than the chain allows', async () => {
      ...
      // Five unrelated operators, so the nomination limit is the only thing
      // binding. The names have to be genuinely unlike each other rather than
      // merely distinct: `Operator A … Operator E` is one operator labelling its
      // own nodes as far as cluster grouping is concerned, and the cluster limit
      // would cut this to two before the nomination limit ever applied.
      const { env } = await loadValidators(set, {
        [one]: 'Zug Capital',
        [two]: 'P2P.ORG',
        [three]: 'RYABINA',
