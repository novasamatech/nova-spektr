...
- **Rendering accounts by `accountId`** — always use `<NamedAccount>` from `@/widgets/NameResolver`. It internalizes `useAccountName` and `useWalletName` so the name shown stays consistent across the app. **Pass `wallet={wallet}` when the account belongs to a local wallet** — this runs the full chain `custom-account-name → local contact → backend contact → identity → wallet.name` and feeds the result through as a name fallback. The `title` prop is an *override* (returns immediately, skipping all resolution — see `accountService.resolveAccountName`); reserve it for cases where the caller has *already* resolved the right name and wants to force it. The raw `<Account>` from `@/shared/ui-entities` is presentational only.
