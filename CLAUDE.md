...
- **Never `memo()` slot-injected components** — the slot render system calls `slotHandlerBody.render(props)` directly as a function. `memo()` returns an exotic object, not a callable function, causing `TypeError: slotHandlerBody.render is not a function`. Only wrap with `memo()` components used as JSX (`<Comp />`), not those passed as `render:` in `feature.inject(slot, { render: Comp })`. (See "No `React.` namespace" above for general `memo` import rules.)
